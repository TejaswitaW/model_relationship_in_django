...
def show_user_data(request):
    ...
    data2 = User.objects.filter(email='kush@gmail.com')
    # use of related name
    data3 = User.objects.filter(mypage__page_cat="Programming")
    data4 = User.objects.filter(
        post__post_publish_date='2020-05-28'
    )
...
def show_post_data(request):
    ...
    data2 = Post.objects.filter(post_publish_date='2020-05-08')
...
def show_page_data(request):
    ...
    data3 = Page.objects.filter(user__email='kush@gmail.com')
...
def show_song_data(request):
    ...
    data3 = Song.objects.filter(user__username='kush')
